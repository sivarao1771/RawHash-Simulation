...
const uint64_t *ri_idx_get(const ri_idx_t *ri, uint64_t hashval, int *n){
    ...
    uint64_t query_hash = hashval >> RI_HASH_SHIFT;
    uint64_t matches[256];  // temporary buffer
    int num = ri_idx_cam_search(query_hash, matches, 256);
    
    for (int i = 0; i < num; ++i) {
        cam_matches.push_back(matches[i]);
    }
    *n = (int)cam_matches.size();
    if (*n == 0) return nullptr;
    return cam_matches.data();
	#endif
...
// #ifdef USE_MEMRISTOR
//     if (g_cam) {
//         static std::vector<uint64_t> match_buffer;
//         match_buffer.clear();

//         std::bitset<64> query(hashval >> RI_HASH_SHIFT);
//         auto matches = g_cam->search(query);

//         for (uint64_t meta : matches) {
//             match_buffer.push_back(meta);
//         }
//         *n = (int)match_buffer.size();
//         if (*n == 0) return nullptr;
//         return match_buffer.data();
//     }
// #endif

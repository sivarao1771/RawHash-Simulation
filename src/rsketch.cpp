...
static MemristorCrossbar* g_crossbar = nullptr;
#endif
...
#ifdef USE_MEMRISTOR
void ri_sketch_crossbar(void *km,
                        const float* s_values,
                        uint32_t id,
                        int strand,
                        uint32_t len,
...
{
	#ifdef USE_MEMRISTOR
    ri_sketch_crossbar(km, s_values, id, strand, len, diff, w, e, n, quant_bit, k,
                       fine_min, fine_max, fine_range, p, out);
    return;
 	#endif
